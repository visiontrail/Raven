...
import { type AIServiceAgentKind, backendToAgentKind, type ProjectRepoOption } from '@renderer/types/aiServiceAgent'
...
const AgentChatWorkbench: FC = () => {
  ...
  const { client, config, profile, logout } = useRavenAccount()
  ...
  const loadProjectRepos = useCallback(async () => {
    ...
    try {
      setProjectRepos(await client.listProjectRepos())
    } catch {
      setProjectRepos([])
    } finally {
  ...
  useEffect(() => {
    if (!profile) return
    conversationStore.attachUser(profile.id)
    void conversationStore.loadSessions()
    void loadProjectRepos()
  }, [loadProjectRepos, profile])
