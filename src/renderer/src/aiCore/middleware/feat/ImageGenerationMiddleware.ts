        ...
        try {
          ...
          const userImages = await Promise.all(
            userImageBlocks.map(async (block) => {
              ...
              return await toFile(new Blob([binaryData.buffer instanceof ArrayBuffer ? binaryData.buffer : new ArrayBuffer(0)]), block.file.origin_name || 'image.png', { type: mimeType })
            })
          )
