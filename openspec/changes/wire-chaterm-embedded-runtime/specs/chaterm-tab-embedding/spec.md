...
- 首次创建时 `src` SHALL 先指向 `about:blank`；只有 `ChatermProcessService.attachWebview()` 成功后，Host 才能把同一个 webview 导航到 Raven 注册的本地协议 URL（如 `raven-chaterm://app/index.html`），不允许加载远程 URL
...
- 加载完成判定 MUST 基于 webview 的 `dom-ready` 与 `did-finish-load` 两个事件**首次触发胜出**（whichever-first）：嵌入态下 `did-finish-load` 受 lazy chunk / pending IPC 影响可能延迟数秒甚至不触发，而 `dom-ready` 在文档解析完即触发，是更可靠的"已可见"信号；该判定只控制遮罩，不得作为 `attachWebview()` 的触发时机

#### Scenario: 首次打开 Terminal 标签

- **WHEN** 用户首次进入 `/terminal`
- **THEN** webview 先以 `about:blank` 创建，Raven MUST 在导航到 `raven-chaterm://app/index.html` 前完成 `attachWebview()`，随后 `dom-ready` 或 `did-finish-load` 任一事件首次触发后加载遮罩 MUST 立即隐藏，Chaterm 主界面渲染完成

#### Scenario: attach 先于 Chaterm SPA 执行

- **WHEN** webview 还停留在 `about:blank`
- **THEN** Raven MUST 已取得 guest `webContentsId` 并调用 `ChatermProcessService.attachWebview()`；只有 attach 成功后才导航到 Chaterm URL，确保 Chaterm renderer 的 `window.api.initUserDatabase` 不会早于主进程 handler 注册
...
`ChatermProcessService.attachWebview()` MUST 遵循两步式握手：

1. **mount**：`await mount({ webContentsId, llmClient?, signals })` 触发 Chaterm `mountChaterm()`，在 `ipcMain` 上注册所有 Chaterm 内部 handler（含 sender 校验）；
2. **registerAllowedSender**：`bridge.registerAllowedSender(webContentsId)` 把 webview 加入 `raven:llm:*` allowlist；

`mountChaterm()` MUST NOT call `bridge.registerAllowedSender()` itself；Raven bridge allowlist 只由 `ChatermProcessService` 持有和修改。

`detachWebview()` MUST 反向执行：`unregisterAllowedSender` → `unmountChaterm()` 撤销所有 disposer。任一步异常 MUST 被捕获并记录，不影响后续步骤的执行。
...
- **THEN** Raven MUST 先调用 `detachWebview('render-process-gone')` 反向执行握手，然后 webview 重新创建并触发新的 blank-first attach；MUST NOT 因 handler 残留而抛 `Attempted to register a second handler`

#### Scenario: attachWebview 中途失败

- **WHEN** `mount` 成功但 `registerAllowedSender` 抛错
